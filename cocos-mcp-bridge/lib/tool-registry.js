...
const { createImageGenTools } = require('./tools/image-gen');
  ...
  const tools = [
    {
      ...
      inputSchema: createSchema(
        {
          context: { type: 'string', description: 'Execution context: "scene" (live runtime, cc.* available: cc.find/cc.color/cc.Node/Graphics) or "editor" (Editor APIs only, NO cc — use scene for any cc.* call).' },
    ...
    },
    {
      ...
      profile: 'full',
      description: '[core] Create a recommended local Codex project skill for Cocos MCP Bridge workflows.',
      inputSchema: createSchema(
        {
    ...
    ...createImageGenTools({ createSchema, getRuntimeContext }),
    {
