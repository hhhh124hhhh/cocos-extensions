...
const LOG_PREFIX = '[Cocos MCP Bridge Server]';
...
class McpServer {
  ...
  async tryAttachToExisting(port) {
    ...
    if (!probe || !probe.result) {
      this.log('warn', `Port ${port} is occupied, but no compatible Cocos MCP Bridge initialize response was received.`);
      return false;
    }
