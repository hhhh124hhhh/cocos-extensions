...
/**
 * Codely Panel — iframe 嵌入 dsh web 聊天界面
 * ------------------------------------------------------------------
 * 从 cocos-codely 迁移而来。面板 iframe 嵌入 dsh web @ 3080，
 * 带 boot mask 和轮询探活逻辑。
 */
...
module.exports = Editor.Panel.define({
  ...
  ready() {
      ...
      if (attempts > MAX_ATTEMPTS) {
        if (hint) {
          hint.textContent =
            ...
            '或手动 `dsh --profile web --patch <cocos-codely>/dsh-cocos-mount.patch.yml`），然后重开面板。';
        }
        return;
      }
    ...
    console.log('[cocos-mcp-bridge:codely] iframe 指向', DSH_WEB_URL);
    poll();
  },
  close() {},
});
