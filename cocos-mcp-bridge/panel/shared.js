...
function settingsTemplate() {
  return `
    <div class="mcp-root">
      ...
      <section class="section">
        <div class="section-title" data-i18n="settings.image_gen">Image Generation</div>
        <div class="settings-grid">
          <label class="field">
...
function createMethods(mode) {
  return {
    ...
    renderState() {
      ...
      this.renderActivity();
      if (this.$.volcArkApiKeyInput) {
        // 出于安全，不在输入框回显真实 key；仅提示是否已设置
        this.$.volcArkApiKeyInput.value = '';
        this.$.volcArkApiKeyInput.placeholder = config.volcArkApiKey
