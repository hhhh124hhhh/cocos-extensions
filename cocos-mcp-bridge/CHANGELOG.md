...
All notable changes to Cocos MCP Bridge will be documented in this file.
...
- Added an npm-installable `cocos-mcp-bridge` stdio wrapper that bridges MCP clients to the local Cocos HTTP endpoint.
...
- `Cocos AI > MCP Server` editor panel for service management and one-click MCP client configuration.
...
- Changed the menu entry to `Cocos AI > MCP Server`.
