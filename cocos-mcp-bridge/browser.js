...
const EXTENSION_NAME = manifest.name || 'cocos-mcp-bridge';
const LOG_PREFIX = '[Cocos MCP Bridge]';
const REPOSITORY_URL = 'https://github.com/ai-game-studio/cocos-mcp-bridge';
...
class ExtensionService {
  ...
  load() {
    ...
    // [fork] 已剥离启动期自动更新检查（sovereign build, no phone-home）。
    return result;
  }
  ...
  async startServer() {
    ...
    this.server = new McpServer({
      ...
      serverName: `Cocos MCP Bridge - ${getProjectName()}`,
  ...
  async checkUpdates(options = {}) {
    this.ensureRuntime();
    this.log('info', 'Checking for newer Cocos MCP Bridge releases.');
  ...
  async installUpdate(options = {}) {
    ...
    this.log(
      'info',
      `Installed Cocos MCP Bridge ${installResult.installedVersion}; ` +
  ...
  scheduleExtensionReload(delayMs = 1200) {
    ...
    setTimeout(async () => {
      try {
        this.log('info', 'Reloading Cocos MCP Bridge after update installation.');
  ...
  async executeEditorScript(payload, runtimeContext) {
    ...
    try {
      ...
      if (/\bcc\b/.test(msg) && /is not (a |an )?(function|defined)/.test(msg)) {
        throw new Error(
          `${msg}\n\n[HINT] 'cc' is not available in execute_javascript context:"editor" ` +
          '(editor context only exposes Editor / Editor.Message / require / fs / path / os). ' +
          'Use context:"scene" for runtime engine APIs (cc.find / cc.color / cc.Node / Graphics).'
        );
      }
      throw err;
    }
  ...
  getClientConfig() {
    ...
    return {
      url,
      codex: `[mcp_servers.cocos_mcp_bridge]\nurl = "${url}"\n`,
      json: JSON.stringify({
        mcpServers: {
          cocos_mcp_bridge: {
            url,
