...
Generated from `lib/tool-registry.js`. The default `core` profile exposes 38 tools; the `full` profile exposes 106 tools.
...
| Tool | Profiles | Access | Description |
|---|---|---|---|
| `create_cocos_mcp_project_skill` | `full` | stateful | [core] Create a recommended local Codex project skill for Cocos MCP Bridge workflows. |
...
| Tool | Profiles | Access | Description |
|---|---|---|---|
| `generate_image` | `full` | stateful | [image] Generate any image (concept/scene/promo/UI), write Cocos .meta and save. ui_mode=true uses UI-specific composition. Requires VOLC_ARK_API_KEY. |
...
| `generate_sprite` | `full` | stateful | [image] Generate a 2D game sprite (character/enemy/prop/avatar), write Cocos .meta and save into project assets/, returning the spriteFrame uuid for scene/prefab wiring. Requires VOLC_ARK_API_KEY. |
