...
Cocos MCP Bridge 是一款面向 **Cocos Creator 3.8 及以上版本**的开源 AI 开发插件。它将 MCP Server 直接嵌入 Cocos Creator 编辑器，让 Claude Code、Cursor、Codex、VS Code Copilot、Trae、Kiro 等支持 MCP 的 AI 编程助手连接当前项目，读取真实编辑器上下文并执行开发工作流。
...
1. **安装插件**
   ...
   从 Cocos Store 将 Cocos MCP Bridge 安装到目标项目，然后使用 Cocos Creator 3.8 或更高版本打开该项目。
...
5. **确认连接**
   ...
   回到 AI 客户端，确认 `cocos-mcp-bridge` 已连接并能看到工具列表。首次使用可以先调用 `get_project_info`、`get_scene_info` 或 `get_hierarchy` 检查项目和场景信息。
...
7. **按需调整工具范围**
   ...
   默认 `core` Profile 适合常用检查和开发任务。如果需要文件写入、完整场景编辑、预览切换等更多能力，可在 `Cocos AI > Tool Exposure` 中切换到 `full` 或创建自定义 Profile。
