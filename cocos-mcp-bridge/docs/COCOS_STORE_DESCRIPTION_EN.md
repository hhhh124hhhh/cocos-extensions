...
Cocos MCP Bridge is an open-source AI development extension for **Cocos Creator 3.8 and later**. It embeds an MCP Server directly in the Cocos Creator editor, allowing MCP-compatible coding assistants such as Claude Code, Cursor, Codex, VS Code Copilot, Trae, and Kiro to connect to the active project, read real editor context, and execute development workflows.

Unlike an assistant that can only analyze project files, Cocos MCP Bridge can inspect the active scene, node hierarchy, components, assets, prefabs, logs, script diagnostics, editor state, and screenshots. This lets AI work from the project's actual state and helps reduce repeated switching between the editor, terminal, and AI client.
...
1. **Install the extension**
   ...
   Install Cocos MCP Bridge from Cocos Store into the target project, then open the project with Cocos Creator 3.8 or later.
...
5. **Confirm the connection**
   ...
   Return to the AI client and confirm that `cocos-mcp-bridge` is connected and its tools are available. For a first test, call `get_project_info`, `get_scene_info`, or `get_hierarchy` to inspect the active project and scene.
...
7. **Adjust tool exposure when needed**
   ...
   The default `core` profile is suitable for common inspection and development tasks. For file writes, complete scene editing, preview switching, and other advanced operations, open `Cocos AI > Tool Exposure` and select `full` or create a custom profile.
