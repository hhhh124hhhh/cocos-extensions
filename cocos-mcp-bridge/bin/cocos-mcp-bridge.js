...
async function main() {
  ...
  if (hasFlag(args, '--version')) {
    console.error(`cocos-mcp-bridge ${readPackageVersion()}`);
    return 0;
  }

  const urlText = getOption(args, '--url')
    || process.env.COOS_MCP_BRIDGE_URL
    || process.env.COCOC_MCP_BRIDGE_URL
    || DEFAULT_URL;
  const timeoutText = getOption(args, '--timeout-seconds')
    || process.env.COCOC_MCP_BRIDGE_TIMEOUT_SECONDS
  ...
  console.error(`[Cocos MCP Bridge] Bridging stdio to ${endpoint.href}`);
...
async function bridgeStdioToHttp({ input, output, endpoint, timeoutMs }) {
  ...
  while (true) {
    ...
    try {
      ...
      console.error(`[Cocos MCP Bridge] ${error.message}`);
      if (requestId !== null) {
...
function printHelp() {
  console.error('cocos-mcp-bridge');
  ...
  console.error('  COCOS_MCP_BRIDGE_URL');
  console.error('  COCOS_MCP_BRIDGE_TIMEOUT_SECONDS');
}
