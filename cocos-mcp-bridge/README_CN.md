...
Cocos MCP Bridge 是一个采用 MIT 协议的 Cocos Creator 扩展，它把 HTTP MCP Server 直接嵌入编辑器，让 Claude Code、Cursor、Codex、VS Code Copilot、Trae、Kiro 等 AI 助手可以直接检查和操作正在运行的 Cocos 项目。
...
如果不想用 git 安装，可以从 GitHub Releases 下载 `Funplay.CocosMcp.v<version>.zip`，解压后把 `cocos-mcp-bridge` 目录移动到项目的 `extensions/` 目录。
...
扩展菜单和原生窗口标题会跟随 Cocos Creator 的界面语言。窗口内的控件、工具描述、状态提示、确认信息和空状态支持中文与英文；打开 `Cocos AI > MCP Settings` 可以跟随 Creator，也可以为当前项目固定内容语言。MCP 工具 ID 和 schema 保持英文，确保客户端集成稳定。

### 3. 配置 AI 客户端

优先使用 `Cocos AI > MCP Server` 面板里的 **MCP Client Config** 区域。
...
这个 wrapper 会把 stdio MCP 流量桥接到 Cocos 内置 HTTP endpoint。也可以直接运行 `npx cocos-mcp-bridge --url http://127.0.0.1:8765/`。
...
- **Cocos 图形面板** — `Cocos AI > MCP Server` 是精简 Dashboard，并提供 Tool Exposure、MCP Settings、Activity 子窗口承载复杂工作流
- **截图与输入支持** — 支持编辑器/场景/Game/Preview 截图，以及 Electron 级鼠标键盘事件
- **厂商无关** — 兼容任意支持 HTTP JSON-RPC MCP 的 AI 客户端

## 与 Funplay MCP for Unity 的关系

Cocos MCP Bridge 延续 Funplay MCP for Unity 的设计原则，并针对 Cocos Creator 的 JavaScript/TypeScript 编辑器环境做了适配。

| 维度 | Cocos MCP Bridge | Funplay MCP for Unity |
|------|------------------------|------------------------|
...
Cocos MCP Bridge 当前在 `full` profile 下提供 **105 个工具函数**：
...
在 Cocos 项目根目录放置 `cocos-mcp-bridge.config.json`：
